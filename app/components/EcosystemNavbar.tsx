...
type Status = {
  ...
  pool?: { managed_by: string; available_agent_slots: number | null };
  updated_at?: string;
};
...
export function EcosystemNavbar() {
  ...
  const agentSlots = status?.pool?.available_agent_slots;
  ...
  return <nav className="flex items-center gap-1 min-h-10 px-4 border-b border-bolt-elements-borderColor bg-bolt-elements-background-depth-2" aria-label="Ecosystem model status">
    ...
    <Pip label="VL 450M" state={!gateway?.reachable ? 'red' : queue?.available_vision_slots === 0 ? 'orange' : 'green'} details={[`State: ${queue?.available_vision_slots === 0 ? 'busy' : running > 0 ? 'hot running' : 'starting'}`, `Running lanes: ${running}/${maxVision}`, `Queue: ${queue?.queued_requests ?? 'unknown'}`]} />
    <Pip label="Agents" state={!gateway?.reachable ? 'red' : agentBusy ? 'orange' : 'green'} details={[`Managed by: Alpha`, `Available lanes: ${agentSlots ?? 'unknown'}`, `Active requests: ${queue?.active_requests ?? 'unknown'}`, `Queue: ${queue?.queued_requests ?? 'unknown'}`]} />
